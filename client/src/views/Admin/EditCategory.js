...
const EditCategory = () => {
  ...
  } = useGetCategoryDetails(params.id, { enabled: !!params.id });
  const { isLoading: isLoadingEdit, mutateAsync: editCategory } =
    useUpdateCategory();

  const dataDetails = !isLoadingDetail ? resCategoryDetails?.data.category : {};
